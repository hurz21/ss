Public Class clsString
  ...
  Public Shared Function isinarray(ByVal userrechteString As string,    ByRef erforderlichesRecht As string,    ByVal delim As string) As Boolean
    'prüft ob t in o als element vorkommt
    Dim i%, s$()
    Try
      isinarray = False
      If Len(userrechteString$) > 1 Then
        s = userrechteString$.Split(CChar(delim$))
        For i = 0 To CInt(UBound(s$))
          If erforderlichesRecht$ = s$(i) Then
            isinarray = True
